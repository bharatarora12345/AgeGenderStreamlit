...
if select_Process == "Browse Image":
    ...
    if uploaded_file is not None:
        ...
        # cv2.imwrite('temp.jpg',cv2.cvtColor(cap , cv2.COLOR_BGR2RGB))
        cap = cv2.imread('temp.jpg')
...
# Third Process for Youtube Videos
if select_Process == "Browse Videos":
    start_Camera = st.selectbox("Select one : Start or Stop",("STOP" ,"START"))
    if(start_Camera == "START"):
        parser = argparse.ArgumentParser()
        parser.add_argument('--image')
        parser.add_argument('-f')
        args = parser.parse_args()
        source_txt = st.text_input('Enter URL :')
        # sample_first = "https://youtu.be/mSnwe7VMjJo"

        # sample_url = st.selectbox("Select URL" ,(""))
